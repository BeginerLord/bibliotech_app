...
export const getAllCategories = async (page: number = 0, size: number = 10, sortBy: string = "name", direction: string = "asc") => {
...
export const updateCategoryByUuid = async (uuid: string, category: CategoryModelDto) => {
    const { data } = await bibliotechapi.put(`/category/update/${uuid}`, category);
    return data as CategoryModel;
}
...
export const getCategoryByDescription = async (description: string) => {
    const { data } = await bibliotechapi.get(`/category/description/${description}`);
    return data as CategoryModel;
}
